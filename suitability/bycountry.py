...
def get_values(rr, cc):
    ...
    unharvbase = values[0] - values[3] if values[0] > values[3] else 0
    return np.array([values[0], posval, negval, confval, values[3], harvloss, posconf, negconf, harvconf, unharvbase])

totals = np.zeros(11)
with open(output_path, 'w') as fp:
    writer = csv.writer(fp)
    writer.writerow(['country', 'baseline', 'increase', 'decrease', 'avgconf', 'lossperc', 'chngperc', 'harvest', 'harvlossperc', 'incconf', 'decconf', 'harvconf', 'unharvbase'])
    ...
    for record in sf.iterRecords():
        ...
        areahas = geoshapes.weighted_grid_within(multi, latitude, longitude, get_values, shape=(10,))
        ...
        writer.writerow([record[name_index], # Country
                         ...
                         int(round(ahnegval)), # Baseline area lost (Ha)
                         "%.3f" % (ahconfval / (ahposval - ahnegval)), # Average confidence
                         round(100 * ahnegval / ahbaseline, 1), # Baseline area lost (%)
                         round(100 * (ahposval + ahnegval) / ahbaseline, 1), # Total change from baseline (%)
                         int(round(ahharvloss)) if np.isfinite(ahharvloss) else 'NA', # Area of harvest lost (Ha)
                         round(100 * ahharvloss / ahharvest, 1), # Area of harvest lost (%)
                         "%.3f" % (ahposconf / ahposval), # Confidence of increase
                         "%.3f" % (ahnegconf / -ahnegval), # Confidence of decrease
                         "%.3f" % (100 * ahharvconf / ahharvest), # Confidence of harvest change
                         int(round(ahunharvbase))]) # Area unharvested of baseline (Ha)

        mytotals = np.array([int(round(ahbaseline)), int(round(ahposval)), int(round(ahnegval)), round(ahbaseline) * (ahconfval / (ahposval - ahnegval)), round(100 * (ahposval + ahnegval) / ahbaseline, 1), ahharvest, ahharvloss, round(ahbaseline) * (ahconfval / ahposval), round(ahbaseline) * (ahconfval / -ahnegval), ahharvconf, ahunharvbase])
        mytotals[np.isnan(mytotals)] = 0
        totals += mytotals

    writer.writerow(["World"] + list(totals[0:3]) + ["%.3f" % (totals[3] / totals[0]), round(100 * totals[2] / totals[0], 1), round(100 * (totals[1] + totals[2]) / totals[0], 1), totals[4], round(100 * totals[6] / totals[5], 1), "%.3f" % (totals[7] / totals[0]), "%.3f" % (totals[8] / totals[0]), "%.3f" % (totals[9] / totals[5]), totals[10]])
